...
from assetforge.services.collapse_impact import SUPPORTED_COLLAPSE_IMPACT_EXTENSIONS


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a collapse-impact diagnostic heatmap.")
    parser.add_argument("source_file", type=Path)
    parser.add_argument("--output", type=Path, default=Path("collapse_impact"))
...
def main() -> int:
    ...
    report_path = args.json or output_directory / "collapse_impact_report.json"
    script_path = (
        Path(__file__).parents[1]
        / "blender"
        / "scripts"
        / "generate_collapse_impact.py"
    )
    executor = BlenderBackgroundExecutor(
        config,
        BlenderLocator(
            UserConfigStore(config.user_config_path),
            explicit_executable=config.blender_executable,
        ),
    )
    try:
